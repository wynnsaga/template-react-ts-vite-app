...
export function NavBar({ items, className }: NavBarProps) {
    ...
    return (
        <div className={cn("fixed bottom-0 sm:top-0 left-1/2 -translate-x-1/2 z-50 mb-6 sm:pt-6", className)}>
            <div className="flex items-center gap-3 bg-white border border-border g py-1 px-1 rounded-full shadow-lg">
                {items.map((item) => {
                    ...
                    return (
                        <NavLink
                            ...
                            className={cn(
                                ...
                                isActive && "bg-[#F4F4F5] text-primary"
                            )}>
                            ...
                            {isActive && (
                                <motion.div
                                    layoutId="lamp"
                                    className="absolute inset-0 w-full bg-primary/5 bg-[#F0F0F0] rounded-full -z-10"
                                    initial={false}
                                    transition={{
                                        type: "spring",
                                        stiffness: 300,
